...
@testset "IUSAC line fitting" begin
    ...
    M, inliers = iusac(data, fit_line, line_dist, 2, 0.5; rng=rng)
...
@testset "IUSAC repeatability" begin
    ...
    results = map(seeds) do seed
        iusac(data, fit_line, line_dist, 2, 0.5;
              rng=StableRNG(seed))
    end
    ...
    @test mean(slopes)     ≈ A_TRUE atol=0.05
    @test mean(intercepts) ≈ B_TRUE atol=0.05
...
@testset "IUSAC determinism with fixed seed" begin
    ...
    M1, inliers1 = iusac(data, fit_line, line_dist, 2, 0.5; rng=StableRNG(SEED))
    M2, inliers2 = iusac(data, fit_line, line_dist, 2, 0.5; rng=StableRNG(SEED))
...
@testset "IUSAC eta_b early stopping" begin
    ...
    M, inliers = iusac(data, fit_line, line_dist, 2, 0.5;
                       rng=rng, eta_b=0.5)
    @test abs(M[1] - A_TRUE) < 0.1
    @test abs(M[2] - B_TRUE) < 0.1
...
@testset "IUSAC error conditions" begin
    ...
    @test_logs (:warn, r"could not draw a non-degenerate sample after") try
        iusac(degenerate_data, fit_line, line_dist, 2, 0.5;
              max_trials=1, max_data_trials=1, rng=StableRNG(1))
    catch
    end

    @test_throws ErrorException iusac(degenerate_data, fit_line, line_dist, 2, 0.5;
